...
describe Bookie do
  ...
  it "should raise an ArgumentError if trying to buy negative number of shares" do
    lambda {@bookie.buy_cost(@all_outcomes, @outcome1, -10)}.should raise_error(ArgumentError)
  end 
  
  it "should cost > $0 to buy 1 share" do
     @bookie.buy_cost(@all_outcomes, @outcome1, 1).should be > 1
  end

  it "should cost the same to buy 20 shares at once or 10 share 2 times" do
    
    twenty_share_cost = @bookie.buy_cost(@all_outcomes, @outcome1, 20)
    
    total = 0
    2.times do
      number_of_shares_to_buy = 10
      total += @bookie.buy_cost(@all_outcomes, @outcome1, number_of_shares_to_buy) 
  ...
  it "should cost $... to buy 10 shares of outcome 1" do
    @bookie.buy_cost(@all_outcomes, @outcome1, 10).should be_within(1).of(96)
  end
